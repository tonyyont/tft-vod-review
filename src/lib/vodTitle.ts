...
export function deriveVODTitle(params: { vod: VOD; metadata?: MatchMetadata | null }): string {
  ...
  if (md) {
    ...
    const prefix = Number.isFinite(placement) && placement > 0 ? `#${placement}` : 'Match';
    const aug1 = Array.isArray(md.augments) ? md.augments.find((a) => !!String(a).trim()) : null;
    if (aug1) return `${prefix} • ${aug1}`;
    const tt = topTrait(md);
    if (tt) return `${prefix} • ${tt.name}${tt.numUnits ? ` (${tt.numUnits})` : ''}`;
    return `${prefix} • ${formatShortDate(vod.createdAt)}`;
  }
